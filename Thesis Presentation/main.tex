...
\begin{frame}{Studying neural dynamics in computational models}
	...
	\textbf{Conductance-based models}
	\\
	\uncover<12->{By different combinations of ionic channels we can achive different activities and waveform shapes}
	\uncover<13>{\includegraphics[width=\linewidth]{intro/spike-types model.pdf}}
	}

\only<14-15>{
	\textbf{Conductace-based models\\}
	\uncover<14->{By modeling synapses we can model whole circuits by connecting single modeled neurons.\\}
...
\begin{frame}{Neural stimulation}
	...
	\begin{itemize}
		\item<5-> The change in the system: Invasive / \textcolor<12>{red}{non-invasive}
		\item<6-> The source of the stimulation: 
			\begin{itemize}
				...
				\item<9->{Magnetic}
				\item<10->{Mechanical}
				\item<11->\textcolor<12>{red}{Optical}			
			\end{itemize}
	\end{itemize}	
}
...
\begin{frame}{Motivation and Objectives}
	\begin{enumerate}
		\only<1-6>{
		\item<1-> To explore the sequential nature of neuronal dynamics at distinct description levels.
		\item<2-> To study sequence interval variability constraints and relationships in neural models and living circuits.
		\item<3-> To analyze the feeding CPG of \textit{Lymnaea stagnalis} to provide evidence of the universality of sequential dynamical invariants found in the pyloric CPG by:
		\begin{enumerate}
		...
		\end{enumerate}
		\item<6-> To illustrate the possible functionality of sequential dynamical invariants in biohybrid robotics.
	}
	\only<7-11>{
		\item<7-> To test the capability of CW-NIR laser stimulation to modulate neuronal dynamics by:
		\begin{enumerate}
		...
		\end{enumerate}
		\item<10-> To study the possible biophysical candidates underlying the CW-NIR effect in model simulations.
		\item<11-> To design and implement a new technique for CW-NIR stimulation in closed-loop.
	}
	\end{enumerate}
...
\begin{frame}{Temporal constrains: Sequential Dynamical invariants}
	...
	\begin{figure}[h!]
		\centering
		\movie[label=pyloric video,width=\textwidth,poster=false,autostart=false,showcontrols,loop] 
...
\begin{frame}{Computational Approach: Model description}
	\begin{columns}
		\begin{column}{0.6\textwidth}
			\begin{itemize}
%				\item<1->Feeding CPG Model (Vavoulis et al.)​
