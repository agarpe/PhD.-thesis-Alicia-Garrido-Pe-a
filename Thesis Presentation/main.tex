...
\only<1>{
	\begin{itemize}
		\item FLC-Hybrot: Functional Living CPG Hybrid Robot.
		\item A robot controlled by the living activity.
		\item Engineering solution to show the context of CPG activity.
	\end{itemize}
}
\only<2>{
\centering
...
\only<3>{\centering\includegraphics[width=\textwidth]{invariants/robot/robot_results_invariant.png}}
\only<2->{\fullcite{amaducci_controlling_2021}}
...
\begin{frame}{Model analysis to explore biophysical candidates}
	...
	\only<1>{
		...
		\centering
		$C_m\frac{d{\color{blue}V}}{dt} = I_{inj} - {\color[rgb]{1,0.5,0.06} I_{NaT}} - {\color[rgb]{0.17,0.63,0.17} I_{NaP}} - {\color[rgb]{0.84,0.15,0.16} I_{A}} - {\color[rgb]{0.58,0.4,0.74} I_{D}} - {\color[rgb]{0.55,0.34,0.29} I_{LVA}} - {\color[rgb]{0.89,0.47,0.76} I_{HVA}}$
		\\
		\vspace{5pt}
		\centering
