...
%is  including the interaction between them. Due to, not only the connections that where represented in the diagram of Fig. \ref{fig:CPG diagram}, but also the intrinsic activity of each neuron, the model reproduces the feeding rhythm and the linear relationships 
% of some intervals with the period as described in \parencite{elliott_temporal_1991}. 
...
%The variability presented in these intervals is *** equivalent **** to the one in period interval. It is remarkable that the N3-N2 interval, which contains N1M and N3t burst duration, is the most similar one ***similar variability to the period***?. Although N3t-BD is much more variable than N1M-BD, this last one still exhibits a considerable difference with respect to N2v-BD, presenting N1M a higher variability  ***discutible***. Therefore, N3-N2 interval, which consists of N1M and N3t, is the most similar to the period, since it is the most variable one (composed by the two BD intervals with higher variability). 
% ****habría que nombar estos intervalos entre paréntesis****.
% The distribution found in intervals N2-N1, N3-N1, N3-N2 intervals and N2-N1 delay, is equivalent to the one in period interval, being once again, the N3-N2 interval the more similar one. Hence, the linear relationships are more likely to appear in these intervals whose distribution is closer to the period. ***esto también lo tenemos que discutir*****
...
%Since the ones involving N3t burst duration are the most correlated to the period, interval changes seem to be due to N3t variability.
